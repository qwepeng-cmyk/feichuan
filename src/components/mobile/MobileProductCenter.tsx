...
export default function MobileProductCenter({ 
    ...
    const categoryList = useMemo(() => {
        ...
        return productCenterCategoryIds.map(key => ({
            id: key,
            name: CATEGORY_NAMES[key],
            icon: CATEGORY_ICONS[key]
        }));
    }, [categoriesData, CATEGORY_ICONS, CATEGORY_NAMES]);
    ...
    useEffect(() => {
        if (categoryList.length > 0 && !categoryList.some((category) => category.id === activeCategory)) {
                ...
                {categoryList.map((category) => (
                    <section key={category.id} id={`mobile-${category.id}`} className={styles.categorySection}>
                        <div
                            ...
                            style={category.id === ACCESSORIES_CATEGORY_ID ? { borderLeft: 0, paddingLeft: 0, textAlign: 'center' } : undefined}
                        >
                            <h2>{CATEGORY_NAMES[category.id]}</h2>
                            <div
                                className={styles.accentLine}
                                style={category.id === ACCESSORIES_CATEGORY_ID ? { margin: '10px auto 0' } : undefined}
                            ></div>
                        </div>

                        {category.id === ACCESSORIES_CATEGORY_ID ? (
                            <AccessoryCategoryCards locale={locale} dict={dict} compact showTitle={false} />
