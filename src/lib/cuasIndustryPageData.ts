...
export type CuasProductSet = 'fixed' | 'mobile';

export type CuasDefenseStage = {
  title: string;
  description: string;
  background: string;
...
export type CuasIndustryPageData = {
  handle: string;
  breadcrumbLabel: string;
  stages: [CuasDefenseStage, CuasDefenseStage];
};

const asset = (handle: string, stage: 1 | 2) =>
  `/solutions/cuas-applications/${handle}/stage-${stage}.webp`;

const pages: CuasIndustryPageData[] = [
  {
    ...
    stages: [
      ...
      },
      {
        ...
        description: 'Secures the core zone with a "red-line" automation logic. By utilizing EMC-compliant navigation spoofing, it surgicality redirects threats away from critical assets, ensuring absolute operational integrity for key buildings.',
  ...
  },
  {
    ...
    stages: [
      ...
      {
        title: 'Defeat Units',
        description: 'We provides a graduated response: Directional Jammers at the perimeter force expulsion through signal suppression, while Navigation Spoofing in core zones safely decoys drones to avoid crash risks. Optional Lasers serve as a final line for surgical neutralization, ensuring absolute protection of critical infrastructure through this multi-tier, high-precision mitigation strategy.',
  ...
  },
  {
    ...
    stages: [
      {
        title: 'Strategic Foundation',
        description: "Fixed Strategic Nodes are deployed at high-vantage points or monitoring towers to provide persistent 24/7 unmanned surveillance. The system utilizes radar and RF for wide-area search and pilot localization, with EO/IR for night verification. For a 'zero-intrusion' objective, high-power interference and spoofing create an electronic shield, enabling automated alerts and decisive drone expulsion from core airspace.",
  ...
  },
  {
    ...
    stages: [
      ...
      },
      {
        ...
        description: 'Patrol units utilize vehicle-mounted and man-portable "detect-and-strike" systems. When the grid triangulates a threat, the C2 Center dispatches the nearest mobile team. These units perform "last mile" intervention, surgically neutralizing drones while apprehending pilots to eliminate unauthorized flights at the source.',
    ...
    stages: [
      {
        ...
        description: 'Our Mobile Ensemble integrates covert systems into escort vehicles as hidden command hubs, paired with man-portable units. This enables rapid, synchronized drone detection and neutralization in a low-profile format.',
        background: asset('vip-private-property-airspace-monitoring', 1),
        productSet: 'mobile',
      },
      {
        title: 'Strategic Base',
        description: "RF drone sensors and omnidirectional jammers are installed on rooftops or perimeter walls to create a 24/7 'Electronic Defense Dome.' This system ensure total privacy and automated protection without interfering with the VIP's domestic lifestyle.",
...
export function getCuasIndustryPageData(handle: string, locale = 'en'): CuasIndustryPageData | null {
  const page = byHandle.get(handle);
  return page ? localizeCuasValue(locale, page) : null;
}

export const allCuasIndustryPageHandles = pages.map((page) => page.handle);
