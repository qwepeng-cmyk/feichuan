...
const russianStrings: Record<string, string> = {
  ...
  'Portable C-UAS Field Shield Pro': 'Переносной полевой комплекс C-UAS Pro',
  'Portable C-UAS Field Shield (Pro)': 'Переносной полевой комплекс C-UAS Pro',
  ...
  'UAV Remote ID Recognition System': 'Система распознавания Remote ID БПЛА',
  ...
  'Portable C-UAS Field Shield for temporary site protection':
    'Переносной полевой комплекс C-UAS для временной защиты объекта',
  'Portable C-UAS field unit in a layered Drone Shield configuration':
    'Переносной полевой модуль C-UAS в составе многоуровневой системы защиты',
  ...
  'Low-altitude radar for Drone Defender tracking':
  ...
  'Jammer and platform linkage': 'Связь системы RF-подавления с командной платформой',
  'Portable and vehicle-mounted C-UAS FAQ':
    'Частые вопросы о переносных и автомобильных системах C-UAS',
  ...
  'Compare handheld detectors, hand-carried identification systems, integrated field kits and vehicle-mounted C-UAS configurations for temporary, patrol and mobile operations.':
    'Сравните ручные детекторы, переносные системы идентификации, интегрированные полевые комплекты и автомобильные конфигурации C-UAS для временных постов, патрулирования и мобильных операций.',
  ...
  'Compare mobile and fixed-site Drone Locator configurations using RF direction finding, radar tracking, EO/IR confirmation and command-platform integration.':
    'Сравните мобильные и стационарные конфигурации определения местоположения дронов с RF-пеленгацией, радарным сопровождением, EO/IR-подтверждением и интеграцией с командной платформой.',
  'Compare fixed-site, portable and vehicle-mounted Drone Shield configurations for multi-sensor detection, target tracking, command and coordinated response.':
    'Сравните стационарные, переносные и автомобильные комплексы защиты от дронов с мультисенсорным обнаружением, сопровождением целей, управлением и согласованным реагированием.',
  'Compare Directional RF Jammer and Omni-directional RF Jammer options for fixed-site C-UAS integration, linked control and coordinated response.':
    'Сравните направленные и всенаправленные системы RF-подавления для интеграции в стационарный комплекс C-UAS, связанного управления и согласованного реагирования.',
  'Drone Radar Detection': 'Радарное обнаружение дронов',
  'Portable Drone Detection': 'Переносные системы обнаружения дронов',
  'Drone Detector': 'Детектор дронов',
  'Drone Defender': 'Многоуровневая защита от дронов',
  'Drone Locator': 'Определение местоположения дронов',
  'Drone Shield': 'Комплекс защиты от дронов',
  'Drone Jammer': 'Системы RF-подавления дронов',
};

export function intentText(locale: Locale, value: string) {
  if (locale !== 'ru') return value;
  return russianStrings[value] || value;
}
...
export function localizeIntentLandingConfig(config: IntentLandingConfig, locale: Locale) {
  return localizeValue(locale, config);
}
...
export function buildIntentLandingPageMetadata(
  ...
  const russianSeoTitles: Record<string, string> = {
    'drone-detector': 'Детектор дронов и мультисенсорная система | N-TET',
    'drone-radar-detection': 'Радар обнаружения дронов Ku/X | N-TET',
    'portable-drone-detection': 'Переносные и автомобильные системы C-UAS | N-TET',
    'drone-defender': 'Многоуровневая защита объектов от дронов | N-TET',
    'drone-locator': 'Определение местоположения дронов | N-TET',
    'drone-shield': 'Комплексы защиты от дронов | N-TET',
    'drone-jammer': 'Направленные и всенаправленные системы RF-подавления | N-TET',
  };
