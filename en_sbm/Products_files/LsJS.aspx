...
var LR_c138='';var LR_c139='';var LR_c140='';var LR_pm016='';	if('0'=='1'){LR_showfloat=0;}
	var _lr_helpsrc_on=unescape("http://lr.zoosnet.net/site/94995150/online_cn.gif");
	var _lr_helpalt_on=unescape("If you have any question%2c please click here for live help.");
	var _lr_helpsrc_of=unescape("http%3a%2f%2flr.zoosnet.net%2fsite%2f94995150%2fonline_cn.gif");
...
var data='jid='+LR_js_pm+'&id='+LR_siteid+'&sid='+LR_sid+'&cid='+LR_cid+'&lng='+LR_lng;
	...
	data+='&r='+lr_refer5237;
	data+='&e='+escape(LR_explain)+'&LLGM=FECF&MMGN=FFDG&NMHO=GGDH&AGHJ='+ lr_para3 + '&s=' + LR_sSize;
...
var LR_mini_js_pctype=3;
		var minichathtml1=(typeof(minichathtml)!='function');if(minichathtml1)document.write('<link href="https://mnn.zoosnet.net/js/JS5.css" rel="stylesheet" type="text/css" />');
function if_src() { if (/rv:11.0;|SE 2.X MetaSr 1.0/.test(navigator.userAgent)) { return ''; } return 'src=\"about:blank\"'; }
var LR_MCount1 = 5000;


function cusEsca(str) {
    try {
...
function cusUnesca(str) {
    try {
        return str.replace(/(%u[0-9A-F]{4})|(%[0-9A-F]{2})|(%[0-9A-F])/gi, function (match) {
            try {
                if (match.startsWith('%u')) {
                    var code = parseInt(match.slice(2), 16);
...
var LR_chatkind = -1;
var lr_refer5237 = cusEsca(document.referrer);
if (typeof (lr_refer5236) != 'undefined') lr_refer5237 = cusEsca(lr_refer5236);
var LR_cname = null;
var LR_ccolor = null;
...
if (typeof (LR_explain) == 'undefined') {
    ...
    if (typeof (LiveReception_explain) != 'undefined') {
        LR_explain = cusUnesca(LiveReception_explain);
    }
    if (typeof (LiveReceptionCode_chatexplain_online) != 'undefined') {
        LR_explain = cusUnesca(LiveReceptionCode_chatexplain_online);
    }
}
...
function LR_minisrc() {
    ...
    if (typeof (LR_rurl) == "string") {
        referInfoTemp1 = cusEsca(LR_rurl);
    }
    ...
    if (LR_robot != '') {
        ...
        return LR_robot + '?siteid=' + LR_websiteid + '&sid=' + LR_sid + '&cid=' + LR_cid + '&lng=' + LR_lng + '&keyword=' + LR_skey + '&lastcusname=&webtype=pcmini&p=' + cusEsca(location.href) + '&r=' + referInfoTemp + "&p1=" + (typeof (LR_purl) == "string" ? cusEsca(LR_purl) : "") + "&e=" + (typeof (LR_explain) != 'undefined' && LR_explain != '' ? cusEsca(LR_explain) : "") + "&msg=" + (typeof (LR_msg) != 'undefined' && LR_msg != '' ? cusEsca(LR_msg) : "");

        /*return LR_robot + '?siteid=' + LR_websiteid + '&sid=' + LR_sid + '&cid=' + LR_cid + '&lng=' + LR_lng + '&keyword=' + LR_skey + '&lastcusname=&webtype=pcmini&p=' + cusEsca(location.href) + '&r=' + referInfoTemp + "&p1=" + (typeof (LR_purl) == "string" ? cusEsca(LR_purl) : "");*/
    }
    else {
        if (!LiveReceptionCode_isonline && typeof (LR_pm016) == "string" && LR_pm016.length > 0) {
            return LR_pm016 + '?id=' + LR_websiteid + '&cid=' + LR_cid + '&lng=' + LR_lng + '&sid=' + LR_sid + '&p=' + cusEsca(location.href) + '&r=' + referInfoTemp;
        }

        return LR_sysurl + 'LR/' + (LiveReceptionCode_isonline ? (minichathtml1 ? 'minichat_PC' : 'minichat3') : 'minioffline160714') + '.aspx?id=' + LR_websiteid + '&cid=' + LR_cid + '&lng=' + LR_lng + '&sid=' + LR_sid + '&p=' + cusEsca(location.href) + '&r=' + referInfoTemp;
    }
}
...
function LR_Check_region() {
    if (LR_defineregion) {
        var invitec0 = cusUnesca(LR_ip1) + ' ' + cusUnesca(LR_ip2);
    ...
    if (LR_hidden_region != '') {
        ...
        var ipfrom = cusUnesca(LR_ip1) + ' ' + cusUnesca(LR_ip2);
...
function LR_showminiDiv(islrminimin, data) {
    ...
    if (!islrminimin) {
        lr_skdata = data ? data : '';
        setTimeout("LR_miniframe.location=LR_minisrc() + '&msg=' + cusEsca(LR_msg)+lr_skdata;LR_msg='';LR_msg='';", 100);
    }
}
...
function lr_refer5238() {
    ...
    return '&rf1=' + cusEsca(ur.substring(0, i)) + '&rf2=' + cusEsca(ur.substr(i));
}
...
function openZoosUrl(url, data) {
    ...
    if (url == 'sendnote') {
        url = LR_sysurl + 'LR/Chatwin2.aspx?siteid=' + LR_websiteid + '&cid=' + LR_cid + '&sid=' + LR_sid + '&lng=' + LR_lng + '&p=' + cusEsca(location.href) + lr_refer5238();
    } else {
        url = ((LR_userurl0 && typeof (LR_userurl) != 'undefined') ? LR_userurl : (LR_sysurl + 'LR/Chatpre.aspx')) + '?id=' + LR_websiteid + '&cid=' + LR_cid + '&lng=' + LR_lng + '&sid=' + LR_sid + '&p=' + cusEsca(location.href) + lr_refer5238();
    }
    if (typeof (LR_UserSSL) != 'undefined' && LR_UserSSL && url.charAt(4) == ':') url = url.substring(0, 4) + 's' + url.substring(4, url.length);
    if (!data) {
        if (typeof (LR_explain) != 'undefined' && LR_explain != '') {
            url += '&e=' + cusEsca(cusEsca(LR_explain));
        } else if (typeof (LiveAutoInvite1) != 'undefined') {
            url += '&e=' + cusEsca(cusEsca(LiveAutoInvite1));
        }
    }
    if (typeof (LR_username) != 'undefined') {
        url += '&un=' + cusEsca(LR_username);
    }
    if (typeof (LR_userdata) != 'undefined') {
        url += '&ud=' + cusEsca(LR_userdata);
    }
    if (typeof (LR_ucd) != 'undefined') {
        url += '&ucd=' + cusEsca(LR_ucd);
    }

    if (LR_getCookie('_lr_show_again') == "1") {
        url += '&_lrsaf=' + cusEsca(LR_getCookie('_lr_show_again'));
    }

    url += '&msg=' + cusEsca(LR_msg);
    if (data) url += data;
        ...
        if (command == "read") {
            ...
            if (args == '') {
                ...
                for (o = 0; o < oo.length; o++) {
                    ...
                    if (oo1.length == 2) {
                        if (isNaN(oo1[1])) {
                            var v = 'if(typeof(LR_' + oo1[0] + ') != "undefined"){LR_' + oo1[0] + '=cusUnesca(oo1[1]);}';
                            eval(v);
                        } else {
...
function LiveReceptionCode_BuildChatWin(_lrchatexplain, _lrhelpalt) {
    return ' href="javascript:void(0)"  onclick="openZoosUrl(\'chatwin\',\'&e=' + cusEsca(cusEsca(_lrchatexplain)) + '\');return false;" title="' + cusUnesca(_lrhelpalt) + '" target="_self" ';
}

function LR_GetAutoInvite2() {
    if (LR_invitesearchkey && LR_skey != null && typeof (LiveAutoInvite3) != 'undefined') {
        return LiveAutoInvite3.replace('%SKEY%', cusUnesca(LR_skey));
...
function LR_LS() {
    ...
    function LRedithref1() {
        ...
        for (var i = 0; i < aList.length; i++) {
            ...
            if (href0 && /chatpre.aspx|chatwin.aspx/.test(hrefcheck) && hrefcheck.indexOf('=' + LR_websiteid.toLowerCase()) > -1) {
                ...
                href0 = LRedithref('&p=', cusEsca(location.href), href0);
    ...
    if (_lr_issupport_track) {
        if (LR_sidexists == 0) {
            ...
            data += '&cn=' + cusEsca(LR_cname) + '&co=' + cusEsca(LR_ccolor);
            checkcount = -1;
            LR_inviteimgJS = 0;
    ...
    if (!event.persisted) {
        if (lr_unloadType == 0) {
            ...
            if (navigator.sendBeacon)
                navigator.sendBeacon(LR_sysurl + 'LR/ClientEndJS.aspx?id=' + LR_siteid + '&lng=' + LR_lng + '&sid=' + LR_sid);
            else
                LR_hcloopJS(LR_sysurl + 'LR/ClientEndJS.aspx', 'id=' + LR_siteid + '&lng=' + LR_lng + '&sid=' + LR_sid);
...
var _lr_form_submit = function () {
    ...
    LR_hcloopJS("https://zoosforms.zoossoft.com/api/web.ashx?action=submit", '&siteid=' + LR_websiteid + '&sid=' + LR_sid + '&cid=' + LR_cid + '&keyword=' + LR_skey + '&p=' + cusEsca(location.href) + lr_refer5238() + "&ip=" + LR_ip + "&allkey=" + encodeURIComponent(strPostMsg));
};
...
var BindLR_Drag = function () {
    ...
    var draggable = document.getElementById('LR_Drag_Box');

    var draggableLeft = document.getElementById('LR_Drag_Box_Left');

    var dragBox = document.getElementById('LRfloater2');

    dragBox.style.display = "flex";
    dragBox.style.position = "fixed";


    var initLeft = getPosition(dragBox).left;
    var initTop = getPosition(dragBox).top;
    ...
    document.addEventListener('mousemove', function (e) {
        if (isDragging) {
            ...
            var maxTop = window.innerHeight - dragBox.offsetHeight;
            var maxLeft = window.innerWidth - dragBox.offsetWidth;
