    ...
    protected function _field_type_html($config, $cname, $value, $hang, $lie, $field = []) {
        ...
        if ($config['type'] == 1) {
            ...
            $url = '/'.(IS_ADMIN ? SELF.'?c=api' : 'index.php?s=api&c=file').'&m=input_file_list&token='.dr_get_csrf_token().'&siteid='.SITE_ID.'&p='.dr_authcode([
                'size' => $size ? $size : 10,
                'exts' => $exts ? $exts : 'jpg,gif,png,jpeg,webp',
                'count' => 1,
            ...
            $html = '<select class="form-control" name="data['.$cname.']['.$hang.']['.$lie.']">';
            //$arr = explode(',', $config['option']);
            $catid = (int)$config['option'];
            $all = \Phpcmf\Service::L('cache')->get('module-'.SITE_ID.'-share', 'category');

            foreach ($all as $cat) {
                if($cat['pid'] == $catid) 
                $html.= '<option '.($value[$hang][$lie] == $cat['id'] ? 'selected' : '').' value="'.$cat['id'].'">'.$cat['name'].'</option>';
            }
            $html.= '</select>';
        }
