...
class Page {
	...
	public function create_links()
	{
        ...
        if ($this->prev_link !== FALSE && ($this->cur_page !== 1 || $this->compel_prev_page))
        {
            ...
            if ($i === $base_page)
            {
				if($this->prev_tag_class){
					$output .= $this->prev_tag_open.'<a class="'.$this->prev_tag_class.'" href="'.$this->_get_link_url($i).'"'.$attributes.$this->_attr_rel('prev').'>'
        ...
        if ($this->next_link !== FALSE && ($this->cur_page < $num_pages || $this->compel_next_page))
        {
			...
			if($this->next_tag_class)
				$output .= $this->next_tag_open.'<a class="'.$this->next_tag_class.'" href="'.$this->_get_link_url($i).'"'.$attributes
                .$this->_attr_rel('next').'>'
                .dr_lang($this->next_link).'</a>'.$this->next_tag_close;
			else
