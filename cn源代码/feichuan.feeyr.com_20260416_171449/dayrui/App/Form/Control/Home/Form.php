        ...
        return parent::_Save($id, $data, $old,
            ...
            function ($id, $data, $old) {
                ...
                if ($this->form['setting']['notice']['use']) {
                    ...
                    if ($this->form['setting']['notice_mail']) {
                        $arr = explode(',', $this->form['setting']['notice_mail']);
                        $obj = \Phpcmf\Service::L('Notice');
                        if (!method_exists($obj, 'send')) {
                            log_message('error', '请升级到最新版的CMS框架源码');
                        } else {
                            foreach ($arr as $email) {
                                $obj->send(
                                    'email',
                                    'form_email_'.$this->form['table'],
                                    $var,
                                    [
                                        'tomail' => $email
                                    ],
