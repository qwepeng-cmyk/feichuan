...
class Form extends \Phpcmf\Table
{
    ...
    protected function _Clink_tpl($uriprefix, $data) {
        ...
        // 记录的链接菜单
        if ($this->_is_admin_auth('edit')) {
            $rt.= '<label><a href="'.dr_url($uriprefix.'/edit').'&id='.$data['id'].'" class="btn btn-xs red"> <i class="fa fa-edit"></i> '.dr_lang('修改').'</a></label>';
            $clink = $this->_app_clink('form', $data);
            if ($clink) {
                foreach ($clink as $a) {
                    $rt.= ' <label><a class="btn '.$a['color'].' btn-xs" href="'.$a['url'].'"><i class="'.$a['icon'].'"></i> '.dr_lang($a['name']);
