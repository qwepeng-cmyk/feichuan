...
| `restricted` | C 层或敏感资料 | 不可以 |

C 层资料只能用于内部判断，不能进入公开 HTML、Schema、`llms.txt`、sitemap、Firecrawl 抓取或对外文章。
...
4. C 层资料只能进入 `Evidence not used`，不能进入公开内容。
5. 如果 RAG 与竞品/SERP 信息冲突，以公司资料和合规规则优先。
